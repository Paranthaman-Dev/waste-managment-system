...
- Last completed task (2026-08-30 20:00 UTC): Fixed vite ngrok `0.tcp.in.ngrok.io` blocked (`vite.config.ts:16 allowedHosts:true` + `hmr.clientPort:443`), added collector Queue overview `RouteMap` `height 340` (`CollectorDashboard.tsx:265` `available` → pins), fixed resident drag (`BinMap editable={false}` no snap-back) + OSRM straight-line fallback removed `User-Agent` → `AbortSignal.timeout(8000)` (`osrm.ts:28-62`), rebuilt resident side-map `max-w-5xl grid [1.15fr_0.85fr] height 420 MapPinned` (`ResidentDashboard.tsx:369,501`); prior 18:00: resident picker + backend 5 active limit + RouteMap fallback; prior 14:00: Drop-off Sites removal + OSRM trip 250m corridor; builds `1886 499.55kB`.
- Next task: Verify refresh rotation/rate-limiting, cover remaining pytest cases, persist uploads volume, and verify queue/route maps end-to-end with live ngrok.
